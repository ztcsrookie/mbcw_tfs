...
if __name__ == '__main__':
    ...
    for data_set_index in range(num_data_sets):
        ...
        for current_random_seed in range(5):
            for cd_measure in ['kernal']:
                try:
                    CD = CausalDiscovery()
                    # cd_model = lingam.ICALiNGAM(random_state=current_random_seed, max_iter=10000)
                    cd_model = lingam.DirectLiNGAM(random_state=current_random_seed, prior_knowledge=bk_sachs_pkc_matrix,
                                                   apply_prior_knowledge_softly=True, measure=cd_measure)
                    # cd_model = lingam.RCD()
    ...
    np.savetxt('Results/Performance/04_12_multi_cws_sachs_pkc_gt_di.csv', performance_results, delimiter=',', fmt='%.4f')